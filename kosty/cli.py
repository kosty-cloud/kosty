...
@click.pass_context
def cli(ctx, organization, region, max_workers, run_all, output):
...
@common_options
@click.pass_context
def ec2_audit(ctx, days, cpu_threshold, organization, region, max_workers, output):
    """Run complete EC2 audit (cost + security)"""
    from .services.ec2_audit import EC2AuditService
    org, reg, workers = get_effective_params(ctx, organization, region, max_workers)
    service = EC2AuditService()
    executor = ServiceExecutor(service, org, reg, workers)
...
@common_options
@click.pass_context
def ec2_cost_audit(ctx, days, cpu_threshold, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def ec2_security_audit(ctx, days, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def ec2_check_oversized(ctx, cpu_threshold, days, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def ec2_check_stopped(ctx, days, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def ec2_check_idle(ctx, cpu_threshold, days, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def ec2_check_previous_gen(ctx, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def ec2_check_ssh(ctx, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def ec2_check_rdp(ctx, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def ec2_check_db_ports(ctx, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def ec2_check_public_non_web(ctx, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def ec2_check_old_ami(ctx, days, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def ec2_check_imdsv1(ctx, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def ec2_check_unencrypted_ebs(ctx, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def ec2_check_no_backup(ctx, days, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def s3_audit(ctx, days, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def s3_cost_audit(ctx, days, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def s3_security_audit(ctx, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def s3_check_empty(ctx, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def s3_check_incomplete_uploads(ctx, days, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def s3_check_lifecycle_policy(ctx, days, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def s3_check_public_read(ctx, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def s3_check_public_write(ctx, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def s3_check_encryption(ctx, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def s3_check_versioning(ctx, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def s3_check_logging(ctx, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def s3_check_wildcards(ctx, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def s3_check_mfa_delete(ctx, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def rds_audit(ctx, days, cpu_threshold, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def rds_cost_audit(ctx, days, cpu_threshold, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def rds_security_audit(ctx, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def rds_check_oversized(ctx, cpu_threshold, days, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def rds_check_idle(ctx, days, cpu_threshold, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def rds_check_unused_replicas(ctx, days, read_threshold, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def rds_check_multi_az(ctx, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def rds_check_backup_retention(ctx, retention_threshold, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def rds_check_gp2(ctx, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def rds_check_public(ctx, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def rds_check_unencrypted(ctx, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def rds_check_default_username(ctx, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def rds_check_wide_cidr(ctx, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def rds_check_disabled_backups(ctx, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def rds_check_outdated_engine(ctx, months_threshold, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def rds_check_no_ssl(ctx, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def lambda_audit(ctx, days, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def lambda_cost_audit(ctx, days, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def lambda_check_unused(ctx, days, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def lambda_security_audit(ctx, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def lambda_check_memory(ctx, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def lambda_check_timeout(ctx, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def lambda_check_public(ctx, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def ebs_audit(ctx, days, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def ebs_cost_audit(ctx, days, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def ebs_security_audit(ctx, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def ebs_check_orphan(ctx, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def ebs_check_low_io(ctx, iops_threshold, days, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def ebs_check_old_snapshots(ctx, days, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def ebs_check_gp2(ctx, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def ebs_check_unencrypted_orphan(ctx, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def ebs_check_unencrypted_in_use(ctx, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def ebs_check_public_snapshots(ctx, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def ebs_check_no_recent_snapshot(ctx, days, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def iam_audit(ctx, days, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def iam_security_audit(ctx, days, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def iam_check_root_keys(ctx, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def iam_cost_audit(ctx, days, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def iam_check_unused_roles(ctx, days, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def iam_check_inactive_users(ctx, days, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def iam_check_old_keys(ctx, days, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def iam_check_wildcard_policies(ctx, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def iam_check_admin_no_mfa(ctx, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def iam_check_weak_password(ctx, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def iam_check_no_rotation(ctx, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def iam_check_cross_account(ctx, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def eip_audit(ctx, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def eip_cost_audit(ctx, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def eip_security_audit(ctx, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def eip_check_unattached(ctx, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def eip_check_stopped_instances(ctx, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def eip_check_dangerous_sg(ctx, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def lb_audit(ctx, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def lb_cost_audit(ctx, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def lb_security_audit(ctx, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def lb_check_no_targets(ctx, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def lb_check_underutilized(ctx, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def lb_check_classic(ctx, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def lb_check_http_redirect(ctx, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def lb_check_tls(ctx, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def lb_check_access_logs(ctx, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def nat_audit(ctx, days, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def nat_cost_audit(ctx, days, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def nat_security_audit(ctx, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def nat_check_unused(ctx, days, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def nat_check_redundant(ctx, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def sg_audit(ctx, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def sg_cost_audit(ctx, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def sg_security_audit(ctx, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def sg_check_unused(ctx, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def sg_check_ssh_rdp(ctx, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def sg_check_db_ports(ctx, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def sg_check_all_ports(ctx, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def sg_check_complex(ctx, rule_threshold, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def cloudwatch_audit(ctx, days, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def cloudwatch_cost_audit(ctx, days, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def cloudwatch_security_audit(ctx, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def cloudwatch_check_alarms(ctx, days, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def cloudwatch_check_logs(ctx, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def cloudwatch_check_custom_metrics(ctx, days, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def dynamodb_audit(ctx, days, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def dynamodb_cost_audit(ctx, days, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def dynamodb_security_audit(ctx, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def dynamodb_find_idle(ctx, days, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def route53_audit(ctx, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def route53_cost_audit(ctx, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def route53_security_audit(ctx, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def route53_check_unused(ctx, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def apigateway_audit(ctx, days, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def apigateway_cost_audit(ctx, days, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def apigateway_security_audit(ctx, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def apigateway_check_unused(ctx, days, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def backup_audit(ctx, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def backup_cost_audit(ctx, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def backup_security_audit(ctx, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def backup_check_empty(ctx, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def backup_check_cross_region(ctx, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def snapshots_audit(ctx, days, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def snapshots_cost_audit(ctx, days, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def snapshots_security_audit(ctx, organization, region, max_workers, output):
...
@common_options
@click.pass_context
def snapshots_check_old(ctx, days, organization, region, max_workers, output):
    """Find old EBS snapshots"""
    from .services.snapshots_audit import SnapshotsAuditService
    org, reg, workers = get_effective_params(ctx, organization, region, max_workers)
    service = SnapshotsAuditService()
...
@common_options
@click.pass_context
def snapshots_find_old(ctx, days, organization, region, max_workers, output):
...
@click.option('--output', default='console', type=click.Choice(['console', 'json', 'csv', 'all']), help='Output format')
@click.pass_context
def audit(ctx, organization, region, max_workers, output):
    ...
    reg = region or ctx.obj['region']
    workers = max_workers or ctx.obj['max_workers']
    
    scanner = ComprehensiveScanner(org, reg, workers)
    reporter = asyncio.run(scanner.run_comprehensive_scan())
