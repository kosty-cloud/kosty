...
class EIPAuditService:
    ...
    def check_unattached_eips(self, session: boto3.Session, region: str, **kwargs) -> List[Dict[str, Any]]:
            ...
            for eip in response['Addresses']:
                if 'InstanceId' not in eip and 'NetworkInterfaceId' not in eip:
                    results.append({
                        ...
                        'Risk': 'HIGH',
                        'Severity': 'High',
                        'Description': f"Elastic IP {eip.get('PublicIp')} is unattached and costs $43.80/month",
    ...
    def check_eips_on_stopped_instances(self, session: boto3.Session, region: str, **kwargs) -> List[Dict[str, Any]]:
    ...
    def check_eips_with_dangerous_sg_rules(self, session: boto3.Session, region: str, **kwargs) -> List[Dict[str, Any]]:
